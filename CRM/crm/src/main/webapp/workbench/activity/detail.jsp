		...
		$("#saveRemarkBtn").click(function (){
			$.ajax({
				...
				success : function (data){
					//data	{"success":true/false,"ar" : {备注1},{2},{3}}
	...
	function showRemarkList(){
		...
		$.ajax({
			...
			success : function (data){
				...
				//onclick="removeRemark(\''+n.id+'\')"  方法里面传递的参数需要在字符串里（方法处在动态拼接中，硬性规定）
				var html=""
	...
	function removeRemark(id){
		$.ajax({
			...
			success : function (data){
				...
				if(data.success){
					//这种方式不行，记录使用的是before方法，会在原有的数据上直接加上删除刷新后的数据
					//showRemarkList();
					$("#"+id).remove()
				}else {
					alert("删除备注失败")
